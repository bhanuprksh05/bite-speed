...
export const updateIdentity = async (req, res) => {
    ...
    let contactData = [];
    if (emailEntry)
        contactData.push(emailEntry);
    if (phoneEntry)
        contactData.push(phoneEntry);
...
const awaitInsertTillHead = async (contact, emails, phoneNumbers, secondaryContactIds) => {
...
const awaitInsertTillTail = async (contact, emails, phoneNumbers, secondaryContactIds) => {
    ...
    if (nextContact) {
        if (nextContact.email)
            emails.add(nextContact.email);
        if (nextContact.phoneNumber)
            phoneNumbers.add(nextContact.phoneNumber);
